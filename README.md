...
Web này (`hoantien-dautay.vercel.app`) là **web độc lập**: donhang/vitien/danhan
được lưu trong **Upstash Redis riêng** của chính web này, không chia sẻ với
hệ thống `phuongthaovip`/`hoan-vi-web` cũ.

1. Bot/uploader riêng của bạn (project `thudautay`) gửi dữ liệu sang
   `POST https://hoantien-dautay.vercel.app/api/sync-data` mỗi khi có cập nhật,
...
Project bot `thudautay` phải trỏ **đúng cùng** `UPSTASH_REDIS_REST_URL`/`TOKEN`
này (copy y hệt sang biến môi trường của `thudautay`), vì cả 2 project cùng
...
SYNC_SECRET = <chuỗi bí mật, khớp với SYNC_SECRET trên project thudautay>
```
